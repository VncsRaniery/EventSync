...
const CategoryFilter = () => {
  ...
  return (
    <Select onValueChange={(value: string) => onSelectCategory(value)}>
      ...
      <SelectContent>
        <SelectItem value="All" className="py-3 cursor-pointer  focus:bg-primary-50       text-[14px] font-normal leading-[20px]">Todas</SelectItem>

        {categories.map((category) => (
          <SelectItem value={category.name} key={category._id} className="py-3 cursor-pointer  focus:bg-primary-50        text-[14px] font-normal leading-[20px]">
            {category.name}
          </SelectItem>
        ))}
      </SelectContent>
